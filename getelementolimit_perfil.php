...
switch ($idsubcat) {
    case '2':
       //        mysql_query("UPDATE ELEMENTO SET VISITAS=VISITAS+1 WHERE IDSUBCATEGORIA=$idsubcat AND IDELEMENTO=$idelemento", $conexion) or die(json_encode($respuesta));
        ...
        $registros=mysql_query("SELECT id, name, quantity, unit, price  FROM $tabla[$idsubcat] where id=$idelemento limit $limit,10 ", $conexion) or
       ...
       //        mysql_query("UPDATE ELEMENTO SET VISITAS=VISITAS+1 WHERE IDSUBCATEGORIA=$idsubcat AND IDELEMENTO=$idelemento", $conexion) or die(json_encode($respuesta));
         ...
         $registros=mysql_query("SELECT id, name, phone, address FROM $tabla[$idsubcat] where id=$idelemento limit $limit,10", $conexion) or
        die(json_encode($respuesta));
       ...
       //        mysql_query("UPDATE ELEMENTO SET VISITAS=VISITAS+1 WHERE IDSUBCATEGORIA=$idsubcat AND IDELEMENTO=$idelemento", $conexion) or die(json_encode($respuesta));
        ...
        break;
          case '6':
         $registros=mysql_query("SELECT CONCAT( schools.id, school_infos.school_id, academic_grades.id,school_infos.year ) AS idtupla, schools.name, school_infos.year, academic_grades.name AS gradoAcademico, school_infos.turn, school_infos.quota
                                 FROM schools
                                 ...
                                 where CONCAT( schools.id, school_infos.school_id, academic_grades.id,school_infos.year )=$idelemento
                                ", $conexion) or
         die(json_encode($respuesta));
         $filas=array();
            while ($reg=mysql_fetch_assoc($registros))
    ...
    break;
    case '7':
         $registros=mysql_query("SELECT  concat(universities.id, carreers.id) as idtupla, universities.name , carreers.name as carrera, universities.address 
                                from  universities 
                                inner join carreers on  universities.id = carreers.university_id
                                where concat(universities.id, carreers.id)=$idelemento
                                ", $conexion) or
         die(json_encode($respuesta));
         $filas=array();
            while ($reg=mysql_fetch_assoc($registros))
            ...
            foreach ($filas as $key => $value) {
                    # code...}
                $count++;
              //  if($count==$indice){  //modifcando para busqueda
   ...
   case '9':
         $registros=mysql_query("SELECT product_probes.id, 
                                    ...
                                    AND product_probes.id=$idelemento
    ...
    break;
    case '36':
        $registros=mysql_query("SELECT id, code, name, kind FROM $tabla[$idsubcat] where id=$idelemento ", $conexion) or
         die(json_encode($respuesta));
          $filas=array();
            while ($reg=mysql_fetch_assoc($registros))
            {
    ...
    case '13':
         $registros=mysql_query("SELECT delegation_infos.delegation_id, 
                                        ...
                                        inner join delegations on delegation_infos.delegation_id= delegations.id 
    ...
    default://EN CASO DE EXISTIR LA TABLA Y TENGA ID Y NOMBRE SE EJECUTA ESTE CASO
         $registros=mysql_query("SELECT id, name FROM $tabla[$idsubcat] where id =$idelemento ", $conexion) or
         die(json_encode($respuesta));
          $filas=array();
            while ($reg=mysql_fetch_assoc($registros))
            {
